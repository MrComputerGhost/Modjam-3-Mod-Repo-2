...
public class ChunkProviderForbidden implements IChunkProvider {
	...
	public ChunkProviderForbidden(World world, long seed,
	...
	public void populate(IChunkProvider par1IChunkProvider, int par2, int par3) {
		...
		for (int c = 1024; c > 0; c--) {
			...
			if ((worldObj.getBlockId(j2, l3, j5) == 0)
					...
					&& ((worldObj.getBiomeGenForCoords(j2, j5) == ForbiddenLands.EnchantedForest) || (worldObj
							.getBiomeGenForCoords(j2, j5) == ForbiddenLands.EnchantedForestHills))) {
				new WorldGenEnchTree(false)
		...
		for (int c = 512; c > 0; c--) {
			...
			if ((worldObj.getBlockId(j2, l3, j5) == 0)
					&& (worldObj.getBlockId(j2, l3 - 1, j5) == Block.grass.blockID)
					&& (worldObj.getBiomeGenForCoords(j2, j5) == ForbiddenLands.ThornForest)) {
				if (BiomeGenThorns.useOldThornBushes) {
					new WorldGenEvilForest2(true).generate(worldObj, rand, j2,
							l3, j5);
				} else if (!BiomeGenThorns.useOldThornBushes) {
					new WorldGenEvilForest3(ForbiddenBlocks.ThornShrub.blockID)
							.generate(worldObj, rand, j2, l3, j5);
				}
			}
		}
		...
		for (int c = 128; c > 0; c--) {
			...
			int j5 = l + rand.nextInt(16) + 8;
			if ((worldObj.getBlockId(j2, l3, j5) == 0)
					&& (worldObj.getBlockId(j2, l3 - 1, j5) == Block.mycelium.blockID)
					&& (worldObj.getBiomeGenForCoords(j2, j5) == ForbiddenLands.Graveyard)) {
		...
		for (int c = 32; c > 0; c--) {
			...
			if ((worldObj.getBlockId(j2, l3, j5) == 0)
					&& (worldObj.getBlockId(j2, l3 - 1, j5) == Block.mycelium.blockID)
					&& (worldObj.getBiomeGenForCoords(j2, j5) == ForbiddenLands.Graveyard)) {
				new WorldGenCrypt().generate(worldObj, rand, j2, l3, j5);
			}
		}
		...
		for (int c = 1024; c > 0; c--) {
			...
			if ((worldObj.getBlockId(j2, l3, j5) == 0)
					&& (worldObj.getBlockId(j2, l3 - 1, j5) == Block.grass.blockID)
					&& ((worldObj.getBiomeGenForCoords(j2, j5) == ForbiddenLands.ForbiddenHills)
							|| (worldObj.getBiomeGenForCoords(j2, j5) == ForbiddenLands.ThornForest) || (worldObj
							.getBiomeGenForCoords(j2, j5) == (ForbiddenLands.ForbiddenLandsBiome)))) {
